...
export function buildSingleAgentPrompt(master: MasterProfile, question: string, feedbackNotes?: string, history?: TranscriptMessage[]) {
  return [
    ...
    "你的投资范围只允许是链上资产与链上策略。",
    "你只能讨论加密货币、稳定币、链上收益仓位、DeFi 协议头寸与其他可在链上持有或执行的资产。",
    "禁止推荐或配置任何链下资产，例如美股、A 股、港股、黄金、原油、房地产、债券、银行理财或现金类传统产品。",
    "如果用户问题涉及链下资产，你要明确收窄到链上可执行替代方案，不要顺着给出链下配置建议。",
...
export function buildCouncilAnalysisPrompt(master: MasterProfile, question: string, feedbackNotes?: string, history?: TranscriptMessage[]) {
  return [
    ...
    "你只能讨论加密货币、稳定币、链上收益仓位、DeFi 协议头寸与其他可在链上持有或执行的资产。",
    "禁止推荐或配置任何链下资产，例如美股、A 股、港股、黄金、原油、房地产、债券、银行理财或现金类传统产品。",
    "如果议题本身涉及链下资产，你要把回答收敛为链上可执行的替代方案，不要给出链下配置建议。",
...
export function buildChallengePrompt(master: MasterProfile, question: string, challenges: ChallengePoint[], feedbackNotes?: string) {
  return [
    ...
    "只回应结构化争议点，不要复述整场会议。",
    "你的投资范围只允许是链上资产与链上策略。",
    "你只能坚持或修正围绕链上资产的观点，不得把回应扩展到任何链下资产配置。",
...
export function buildSynthesisPrompt(
  ...
  return [
    ...
    "最终方案只能包含链上资产、稳定币与链上可执行策略。",
    "禁止在任何分配、理由或执行步骤里出现股票、债券、黄金、商品、房地产、银行理财、法币存款等链下资产。",
    "如果原始问题夹杂链下资产需求，你要明确说明本委员会只输出链上投资方案，并把结论收敛到链上配置。",

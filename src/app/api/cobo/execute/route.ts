...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await executeCoboOperation(config, body.operation, body.requestId?.trim());
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to execute Cobo operation";
